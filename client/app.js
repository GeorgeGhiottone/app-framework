...
let easierFirebase = {
    computed: {
        ...
        // Realtime Database
        $fireDB() {
            return window.firebase && window.firebase.database ? (ref) => {
                return window.firebase.database().ref(ref)
            } : null
        },
        // Cloud Storage
        $fireStore() {
            return window.firebase && window.firebase.storage ? (ref) => {
...
mixins.loadRoutes = {
    ...
    created: function () {
        ...
        r < routes.length; r++) {
            ...
            // Login required?
            if (routes[r].login === true)
                this.loginRoutes.push(routes[r].path)
            // Tab routes
...
mixins.loadIconFonts = {
    created: function () {
        ...
        if (process.env.FONT_MATERIAL === 'true')
            require('../vendor/material-icons/material-icons.css')
    ...
    watch: {
        language: function (newLanguage, oldLanguage) {
            ...
            if (/^[a-z]{2}$/.test(newLanguage)) {
                ...
                for (let item in useText)
                    window.f7.params[item] = useText[item]
        ...
        color: function (newColor, oldColor) {
            ...
            if (this.colors[this.theme][newColor] !== undefined) {
                ...
                window.Dom7('body')[0].className.split(' ').map(function (cName) {
                    if (/^theme-[a-z]+$/.test(cName))
                        window.Dom7('body').removeClass(cName)
                })
